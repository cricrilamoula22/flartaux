...
from models import db, UnitTasks, Users
...
def index():
    ...
    return render_template('agent/agent.html', agent=current_user, grouped=grouped)

@agent_bp.route('/task/<int:task_id>/move/<string:direction>')
@login_required
def move_task(task_id, direction):
    task = UnitTasks.query.get_or_404(task_id)
    ...
    db.session.commit()
    return redirect(url_for('agent_bp.index'))
