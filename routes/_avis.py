...
@avis_bp.route("/avis", methods=["GET", "POST"])
@login_required
def index():
...
def index():
    return render_template("generate.html")
"""    
@avis_bp.route("/generate", methods=["POST"])
@with_db_connection
def generate(cursor):
    try:
...
@avis_bp.route("/review", methods=["POST"])
@with_db_connection
def review(cursor):
    try:
...
@avis_bp.route("/generate_selected", methods=["POST"])
@with_db_connection
def generate_selected(cursor):
    try:
        ...
        zip_path = generate_odt_and_zip(selected_records, sub_records,
