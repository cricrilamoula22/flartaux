...
#from app import db
#from app.selection.models import db
...
from models import TCommune, TCadastre, SelectionParcelle
...
selection_bp = Blueprint(
    ...
    template_folder="../templates/selection"
)
...
def index():
    ...
    # Récupérer le session_id du formulaire ou de la session
    session_id = request.form.get('session_id') or session.get('session_id')

    # Si un session_id est fourni via le formulaire, le stocker dans la session
    if request.method == 'POST' and 'session_id' in request.form:
        session['session_id'] = session_id
        flash('Session chargée avec succès.', 'success')
    ...
    if session_id:
        ...
        sous_totaux = db.session.query(
            TCommune.code_insee_commune,
            TCommune.libelle_commune,
            func.sum(TCadastre.dcntsf).label('surface_totale')
        ).join(
            TCadastre, TCadastre.idcom == TCommune.code_insee_commune
        ).filter(
        ...
        subtotals = {(code_insee_commune, libelle_commune): surface_totale for code_insee_commune, libelle_commune, surface_totale in sous_totaux}
...
def export_parcelles_excel():
    ...
    # Utiliser le session_id pour nommer le fichier
    return send_file(
        stream,
        as_attachment=True,
        download_name=f"selection_parcelles_{session_id}.xlsx",
