...
def get_category_name(type_id):
    if not type_id:
        return None

    # recherche directe sur id
    cat = TreeCategories.query.get(type_id)
    if cat:
        return cat.name

    # recherche par parent_id
    cat = TreeCategories.query.filter_by(parent_id=type_id).first()
    if cat:
        return cat.name
  ...
  AND date_complet <> ''
    ...
    for no_interne, date_complet, pac_dem, pac_ced, type_demande, motif_ctrl in rows:
        ...
        records.append({
            ...
            # ✅ Ces champs seront remplis plus tard dans main.py
            "type_checked": False,
            "motif_checked": False,
            "type_name": None,
            "motif_name": None
